...
interface AudioManagerProps {
    ...
    onAudioDataChange: (data: AudioBuffer | undefined) => void;
}

export function AudioManager({ transcriber, onAudioDataChange }: AudioManagerProps) {
    const { addTranscription } = useTranscriptionHistory();
    ...
    // Add function to save transcription
    const saveTranscription = (audioData: AudioBuffer, transcriptionData: TranscriberData) => {
        if (!transcriptionData.isBusy) {
            ...
            addTranscription({
                ...
                metadata: {
                    ...
                    isAutoSaved: false,
                    fileSize: audioData.length * 4 // Approximate size in bytes
                }
            });
        }
    };

    // Update parent component when audioData changes
    useEffect(() => {
        onAudioDataChange(audioData?.buffer);
    }, [audioData, onAudioDataChange]);

    return (
        <>
            <div className='flex flex-col justify-center items-center rounded-2xl bg-white dark:bg-gray-900 shadow-xl shadow-black/10 dark:shadow-black/30 ring-1 ring-black/10 dark:ring-white/10 overflow-hidden'>
                <div className='flex flex-row justify-center items-center space-x-4 py-6 w-full px-8'>
                    <UrlTile
                        icon={<AnchorIcon />}
                        text={"From URL"}
                        onUrlUpdate={(e) => {
            ...
            {audioData && (
                <div className='mt-6 w-full space-y-6'>
                            ...
                            onClick={() => {
                                if (audioData) {
                                    ...
                                    const checkCompletion = setInterval(() => {
                                        if (transcriber.output && !transcriber.output.isBusy) {
                                            saveTranscription(audioData.buffer, transcriber.output);
                    ...
                    {transcriber.progressItems.length > 0 && (
                        <div className='relative z-10 p-4 w-full rounded-xl bg-white dark:bg-gray-900 shadow-lg'>
...
function VerticalBar() {
    return <div className='w-[1px] bg-slate-200 dark:bg-slate-700'></div>;
}
...
function ProgressBar(props: { progress: string }) {
    return (
        <div className='w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1'>
            <div
                className='bg-blue-600 dark:bg-blue-500 h-1 rounded-full transition-all duration-100'
...
function Tile(props: {
    ...
    return (
        <button
            ...
            className='flex items-center justify-center rounded-xl p-3 text-gray-700 dark:text-gray-100 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900 transition-all duration-200'
        >
